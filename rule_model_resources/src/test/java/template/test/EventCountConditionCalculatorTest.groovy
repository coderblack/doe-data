...
class EventCountConditionCalculatorTest implements RuleConditionCalculator {
    private Jedis jedis;
    ...
    public boolean isMatch(int guid) {
        ...
        // 取出一个事件条件参数
        JSONObject eventParam_3 = eventParams.getJSONObject(3);
        // 取出该条件的条件id
        Integer conditionId_3 = eventParam_3.getInteger("conditionId");

        // 取出该事件次数条件的要求的发生次数
        Integer eventCountParam_3 = eventParam_3.getInteger("eventCount");

        // 去redis中查询该用户，该条件的实际发生次数
        String realCountStr_3 = jedis.hget(ruleId + ":" + conditionId_3, guid + "");
        int realCount_3 = Integer.parseInt(realCountStr_3 == null ? "0" : realCountStr_3);

        // 判断，该条件是否已经满足
        boolean res_3 = realCount_3 >= eventCountParam_3 ;
