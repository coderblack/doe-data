...
import cn.doitedu.rtmk.engine.utils.UserEventComparator;
...
public class RuleMatchProcessFunction extends KeyedBroadcastProcessFunction<Integer, UserEvent, RuleMetaBean, RuleMatchResult> {
        ...
        for (Map.Entry<String, RuleMetaBean> ruleEntry : ruleEntries) {
            ...
            if (profileUserBitmap.contains(userEvent.getGuid())) {
                ...
                if (UserEventComparator.userEventIsEqualParam(userEvent, ruleTrigEventJsonObject)) {
                    ...
                    boolean isMatch = ruleMetaBean.getRuleConditionCalculator().isMatch(userEvent.getGuid());

                    log.info("用户:{} ,触发事件:{},规则:{},规则匹配结果:{}",userEvent.getGuid(),userEvent.getEventId(),ruleEntry.getKey(),isMatch);
                    // 如果已满足，则输出本规则的触达信息
                    if(isMatch) {
                        out.collect(new RuleMatchResult(userEvent.getGuid(), ruleEntry.getKey(), System.currentTimeMillis()));
                    }
                ...
                // 判断用户行为事件，如果本事件不是规则的触发事件，则进行规则的条件统计运算
                    ...
                    // 做规则运算
                    ruleMetaBean.getRuleConditionCalculator().calc(userEvent);
    ...
    @Override
    public void onTimer(long timestamp, KeyedBroadcastProcessFunction<Integer, UserEvent, RuleMetaBean, RuleMatchResult>.OnTimerContext ctx, Collector<RuleMatchResult> out) throws Exception {
    ...
    public void processBroadcastElement(RuleMetaBean ruleMetaBean, KeyedBroadcastProcessFunction<Integer, UserEvent, RuleMetaBean, RuleMatchResult>.Context ctx, Collector<RuleMatchResult> out) throws Exception {
        ...
        try {
            ...
            if ("INSERT".equals(operateType) || "UPDATE".equals(operateType)) {
                ...
                // 对规则运算器做初始化
                ruleConditionCalculator.init(jedis, JSON.parseObject(ruleMetaBean.getRuleParamJson()));
