...
object UserProfileBulkLoadTest {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 先从数据源（hive）中读取到源数据
    val tagDataframe = spark.read.table("test_bulk_profile")
    /*
        +----+---+---------+---------+----------+
    ...
    val kvRdd: RDD[(ImmutableBytesWritable, KeyValue)] = tagDataframe.rdd.flatMap(row=>{
      val guid = row.getAs[Int]("guid")
      val age = row.getAs[Int]("age")
      val addr = row.getAs[String]("addr")
      val order_amt = row.getAs[Double]("order_amt")

      List(
        (guid,"f","age",age+""),
        (guid,"f","addr",addr),
        (guid,"f","order_amt",order_amt+""),
      )
    })
    ...
    kvRdd.saveAsNewAPIHadoopFile(
      "hdfs://doitedu:8020/up_tags/2022-08-04/",
    ...
    loader.bulkLoad(tableName,new Path("hdfs://doitedu:8020/up_tags/2022-08-04/"))
