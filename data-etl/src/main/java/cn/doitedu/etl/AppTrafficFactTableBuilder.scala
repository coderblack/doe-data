...
case class ResultBean(guid:Long, session_id:String, page_id:String, pv_cnt:Long, page_staytime:Long, pv_contribute_d:Int, pv_contribute_w:Int)
...
object AppTrafficFactTableBuilder {
  ...
  def main(args: Array[String]): Unit = {
     ...
     * PageBean(1846,ZYBFEFJFQW,page001,page023,1658043583601,1658043592901)
    ...
    val result: RDD[ResultBean] = grouped.flatMap(tp => {
      ...
      for (i <- pageloadEventList.indices) {
        ...
        pageStaytimeMap += ((pageloadEventList(i).page_id, pageStaytimeMap.getOrElse(pageloadEventList(i).page_id, 0) + staytime))
       ...
       */
      val resList:ListBuffer[ResultBean] = ListBuffer.empty
      // TODO
